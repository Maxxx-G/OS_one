...
export const useMemoryContext = () => {
  ...
  const build = useCallback(
    async (q: string, topk = 3): Promise<MemoryBlock> => {
      ...
      const fmt = profile?.formatting_prefs
        ? `format=${JSON.stringify(profile.formatting_prefs)}`
        : '';
      const dos =
        Array.isArray(profile?.do_list) && profile.do_list.length
          ? `do=${profile.do_list.join(', ')}`
          : '';
      const donts =
        Array.isArray(profile?.dont_list) && profile.dont_list.length
          ? `dont=${profile.dont_list.join(', ')}`
          : '';
