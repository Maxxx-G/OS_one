...
export const ProfilePanel: React.FC = () => {
  ...
  const parsedFmt = useMemo(() => {
    try { return JSON.parse(fmt || '{}'); } catch { return null; }
  }, [fmt]);

  const onApply = async () => {
    if (!parsedFmt) { setStatus('err'); return; }
    setStatus('saving');
    const result = await update({
      tone,
      formatting_prefs: parsedFmt,
      do_list: dos.split(',').map(s => s.trim()).filter(Boolean),
      dont_list: donts.split(',').map(s => s.trim()).filter(Boolean),
    });
  ...
  return (
    <div className="grid gap-2 p-2 border rounded">
      ...
      <div className="grid gap-2">
        ...
        <input className="border rounded px-2 py-1" value={tone} onChange={e => setTone(e.target.value)} placeholder="neutral|concise|friendly|formal" />
        <label className="text-xs">Formatting Prefs (JSON)</label>
        <textarea className="border rounded p-2 text-xs min-h-[100px]" value={fmt} onChange={e => setFmt(e.target.value)} />
        <label className="text-xs">Do List (comma-separated)</label>
        <input className="border rounded px-2 py-1" value={dos} onChange={e => setDos(e.target.value)} />
        <label className="text-xs">Don't List (comma-separated)</label>
        <input className="border rounded px-2 py-1" value={donts} onChange={e => setDonts(e.target.value)} />
      </div>
      <div className="flex items-center gap-2">
        <button className="border rounded px-2 py-1 text-sm disabled:opacity-50" disabled={disabled || !parsedFmt} onClick={onApply}>
          {status === 'saving' ? 'Saving…' : 'Apply'}
        </button>
        {!token && <span className="text-xs text-gray-500">paste a JWT in AuthBar to enable saves</span>}
